# Atividade Python com Flet

![Status](https://img.shields.io/badge/status-concluído-brightgreen) ![Python](https://img.shields.io/badge/Python-3.x-blue?logo=python) ![Flet](https://img.shields.io/badge/Flet-Framework-purple) ![GitHub](https://img.shields.io/badge/GitHub-Repository-black?logo=github)

Repositório com atividades práticas desenvolvidas em Python utilizando o framework **Flet**, com foco na criação de interfaces gráficas.

---

## Atividades Desenvolvidas

### Atividade 1  Cartão de Apresentação

Foi desenvolvida uma interface de apresentação utilizando componentes de texto do Flet.

A aplicação apresenta o nome e a área profissional, além de utilizar configurações de alinhamento, tamanho da janela, espaçamento e cores personalizadas.

**Resultado**

<div align="center">
  <img src="https://github.com/user-attachments/assets/8473b9a7-e9bc-4be8-a5b1-22c71fc74dde" width="350">
</div>

---

### Atividade 2 Perfil Profissional

Nesta atividade foi desenvolvida uma interface de perfil utilizando diferentes componentes do Flet.

A aplicação apresenta informações profissionais e dados de contato organizados em um cartão visual.

**Componentes utilizados:**

- Container
- Text
- Row
- Column
- Icon
- Cores personalizadas
- Bordas arredondadas

**Resultado**

<div align="center">
  <img src="https://github.com/user-attachments/assets/8edf0a19-01a2-466b-b250-22aacad1791b" width="350">
</div>

---

### Atividade 3  Formulário de Cadastro

Foi desenvolvido um formulário para entrada e validação de informações.

O usuário pode inserir seu nome, selecionar a opção de aceite dos termos e enviar os dados. Após o envio, a aplicação apresenta uma mensagem personalizada.

**Funcionalidades:**

- Campo para nome completo
- Entrada de dados
- Checkbox para aceite dos termos
- Botão de envio
- Validação do campo
- Mensagem de retorno ao usuário

**Resultado**

<div align="center">
  <img src="https://github.com/user-attachments/assets/2aab752a-daac-48b3-b74a-5b8b98bc808c" width="350">
</div>

---

### Atividade 4  Mercado do Mago

Nesta atividade foi desenvolvida uma aplicação para cadastro e controle de itens.

O usuário pode inserir o nome de um item e adicioná-lo à lista. Cada item possui controles para aumentar ou diminuir sua quantidade.

**Funcionalidades:**

- Cadastro de itens
- Campo para inserção do nome do item
- Validação do campo
- Adição dinâmica de itens
- Controle de quantidade
  - Aumento da quantidade
  - Redução da quantidade
- Mensagem de confirmação

**Resultado**

<div align="center">
  <img src="https://github.com/user-attachments/assets/56801454-5224-404a-a308-c61efc1f2ea1" width="350">
</div>

---

## Outros Exercícios

### Contador

O arquivo `main.py` apresenta uma aplicação simples de contador.

A interface possui botões para aumentar e diminuir um valor, permitindo praticar eventos e atualização dinâmica dos componentes.

### Olá Mago

O arquivo `olamundo.py` apresenta um exemplo inicial de aplicação utilizando Flet, exibindo uma mensagem de boas-vindas na interface.

---

## Principais Conceitos Praticados

Durante o desenvolvimento das atividades foram trabalhados conceitos fundamentais de Python e Flet, incluindo:

- Funções
- Variáveis
- Condicionais
- Eventos
- Componentes gráficos
- Entrada de dados
- Validação de informações
- Manipulação de valores
- Atualização da interface
- Organização de componentes

**Componentes do Flet:**

- Text
- TextField
- Checkbox
- Button
- IconButton
- Icon
- Container
- Row
- Column

---

## Como Executar

### 1. Instalar o Python

Verifique se o Python está instalado na máquina:

```bash
python --version
```

### 2. Instalar o Flet

No terminal, execute:

```bash
pip install flet
```

### 3. Clonar o repositório

```bash
git clone https://github.com/PaollaVeronez05/Atividade_Python_Flet.git
```

### 4. Acessar a pasta do projeto

```bash
cd Atividade_Python_Flet
cd Flet
```

### 5. Executar as atividades

```bash
# Primeira atividade
python ativ1.py

# Segunda atividade
python ativ2.py

# Terceira atividade
python ativ3.py

# Quarta atividade
python ativ4.py
```

---

## Funcionalidades por Arquivo

| Arquivo       | Descrição                          |
|---------------|-------------------------------------|
| `ativ1.py`    | Cartão de apresentação              |
| `ativ2.py`    | Perfil profissional                 |
| `ativ3.py`    | Formulário de cadastro              |
| `ativ4.py`    | Cadastro e controle de itens        |
| `main.py`     | Contador interativo                 |
| `olamundo.py` | Exemplo inicial com Flet            |
| `inicio.py`   | Estrutura inicial da aplicação      |

---

## Aprendizados

O desenvolvimento deste projeto proporcionou a prática de conceitos fundamentais de Python aplicados à criação de interfaces gráficas.

A utilização do Flet permitiu trabalhar com componentes visuais, eventos, entrada e validação de dados, além da atualização dos elementos da interface de acordo com as ações realizadas pelo usuário.

As atividades também contribuíram para a compreensão da organização de aplicações Python e para o desenvolvimento de interfaces simples e interativas.

---

## Organização das Imagens

As imagens utilizadas no README devem estar dentro da pasta `imagens`, localizada na raiz do projeto.

Os arquivos devem estar organizados da seguinte forma:

```
imagens/
│
├── cartao-apresentacao.png
├── perfil.png
├── formulario.png
└── mercado-do-mago.png
```

---

## Considerações Finais

A atividade permitiu aplicar conhecimentos de Python na criação de interfaces gráficas utilizando o Flet, explorando diferentes componentes e recursos de interação com o usuário.

O projeto representa uma etapa prática de desenvolvimento e contribui para a construção de conhecimentos relacionados à programação e ao desenvolvimento de aplicações.

---

## Autora

**Paolla Paula Veronez**
Estudante de Desenvolvimento de Sistemas

## Orientadores

- Prof. Paulo Camargo
- Prof. Raul Porto Lopes

## Instituição

**SENAI** — Serviço Nacional de Aprendizagem Industrial
Curso Técnico em Desenvolvimento de Sistemas

## Repositório

O código-fonte completo do projeto está disponível no GitHub:
[Atividade Python com Flet](https://github.com/PaollaVeronez05/Atividade_Python_Flet)

---

*Paolla Paula Veronez — SENAI — Curso Técnico em Desenvolvimento de Sistemas — 2026*
